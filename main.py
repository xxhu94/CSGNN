...
from model import GAT_COBO, CostMatrix
...
parser.add_argument('--lr_GNN', type=float, default=0.001, help='Initial learning rate for GNN.')
parser.add_argument('--lr_Cost', type=float, default=0.001, help='Initial learning rate for Cost matrix.')
parser.add_argument('--weight_decay_GNN', type=float, default=1e-3, help='Weight decay for optimizer(all layers) GNN.')
parser.add_argument('--weight_decay_Cost', type=float, default=1e-3, help='Weight decay for optimizer(all layers) Cost matrix.')
...
for layer in range(args.layers):
    ...
    # use optimizer
    optimizer_GNN = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr_GNN, weight_decay=args.weight_decay_GNN)
    optimizer_Cost = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr_Cost, weight_decay=args.weight_decay_Cost)
